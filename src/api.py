...
def init_prometheus_client():
    ...
    return gauge, request_count_gauge, time_count_guage
...
gauge, request_count_gauge, time_count_guage = init_prometheus_client()
...
def create_custom_gauge_metrics():
    ...
    if reset_counter:
        for key in total_gauge_time.keys() and total_gauge_count:
            ...
            time_count_guage.labels(group_by[0], group_by[1], group_by[2]).set(0)
...
def metrics_colletion():
    ...
    try:
        ...
        time_count_guage.labels(request_method, group, status_code).inc(value)
